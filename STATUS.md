...
- Fixed repo reindex endpoint behavior to return deterministic `repo_not_found` (HTTP 404) for unknown `repo_id` before ingestion starts, preventing internal errors for user-input misses; added API regression checks for both `/repos/reindex-docs` and `/repos/{id}/reindex`.
